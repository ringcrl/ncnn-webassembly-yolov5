...
public:
  ...
  std::vector<pair<cv::Rect, vector<Point2f>>> detect(cv::Mat img,
                                                      int detim_size = 640) {

    std::pair<cv::Mat, float> detIm_scala =
    ...
    for (int j = 0; j < _feat_stride_fpn.size(); j++) {
      ...
      sprintf(clsname, "face_rpn_cls_prob_reshape_stride%d",
              _feat_stride_fpn[j]); // sprintf打印到字符串中,即赋值
      char regname[100];
      sprintf(regname, "face_rpn_bbox_pred_stride%d", _feat_stride_fpn[j]);
      char ptsname[100];
      sprintf(ptsname, "face_rpn_landmark_pred_stride%d", _feat_stride_fpn[j]);
