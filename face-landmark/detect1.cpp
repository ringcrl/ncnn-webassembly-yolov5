...
int main(int args, char **argv) {
  ...
  std::vector<pair<cv::Rect, vector<Point2f>>> faces =
      retina.detect(img_c, 640);

  vector<vector<Point>> all_points;

  for (int i = 0; i < faces.size(); i++) //遍历人脸
  {
    pair<cv::Rect, vector<Point2f>> face =
        faces[i]; //这里的人脸还是在det_im(640x640)图中的坐标

    Rect box = face.first;             // img下人脸框
    vector<Point2f> pts = face.second; // img下的5个特征点

    // cv::rectangle(img_c, cv::Point((int)box.x, (int)box.y),
    // cv::Point((int)box.width, (int)box.height), cv::Scalar(0, 255, 255), 2,
    // 8, 0); for (int j = 0; j < pts.size(); ++j) { 	cv::circle(img_c,
    //cv::Point((int)pts[j].x, (int)pts[j].y), 1, cv::Scalar(225, 0, 225), 2,
    //8);
    // }
