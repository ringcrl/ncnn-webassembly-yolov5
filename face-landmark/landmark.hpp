...
public:
  ...
  vector<cv::Point> get_landmark(cv::Mat img, cv::Rect box) {
    ...
    cv::waitKey(0);
    ncnn::Mat indet =
        ncnn::Mat::from_pixels(rimg.clone().data, ncnn::Mat::PIXEL_BGR,
                               rimg.cols, rimg.rows); // rimg已经是192x192x3
    indet.substract_mean_normalize(pixel_mean2,
                                   pixel_std2); //数据标准化,可有可无
    ex.input("data", indet);
  ...
  ~Landmark() { cout << "landmark 释放" << endl; };
